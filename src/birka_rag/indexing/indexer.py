"""Создание RAG векторной базы из документов с эмбеддингами."""
...
def _load_docs_from_paths(file_paths: list[Path]) -> list[Document]:
...
def _apply_passage_prefix(chunks: list[Document], model_name: str) -> list[Document]:
...
def create_rag_index(
    ...
    if not file_paths:
        raise ValueError(
            "Не найдено подходящих файлов. Проверьте RAG_SOURCE_PATHS, "
            "RAG_RECURSIVE и RAG_SOURCE_EXTENSIONS."
        )
